...
const SOCKET_URL =
  ...
  "https://whatsapp-backend-en5o.onrender.com";
...
export default function Messenger() {
  ...
  // -----------------------------
  // CONNECT SOCKET WHEN USER LOADED
  // -----------------------------
  useEffect(() => {
    if (!user) return;
  ...
  const createGroup = async () => {
    const name = prompt("Group name");
    const emailsStr = prompt("Comma separated emails");

    if (!name || !emailsStr) return;
    ...
    try {
      ...
      alert(err.response?.data?.error || "Group create failed");
    }
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        const upl = await api.post("/upload", form, {
                ...
                <div style={{ fontSize: 12 }}>
                  ...
                  {c.type === "private" && (
                    <span
                      style={{
                        float: "right",
                        color: onlineUsers[otherUser?._id]
                          ? "green"
                          : "gray",
                      }}
                    >
